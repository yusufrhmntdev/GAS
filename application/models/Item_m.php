...
class Item_m extends CI_Model{
   ...
   var $column_search = array('barcode', 'item.name', 'price'); //set column field database for datatable searchable
    ...
    public function kode(){
        $this->db->select('RIGHT(item.barcode,4) as barcode', FALSE);
        ...
        if($query->num_rows() <> 0){      
            ...
            $kodetampil = "GA".$tgl.$batas;  //format kode
            return $kodetampil;  
       }
    ...
    function update_stock_laptop_kembali($data){
        $qty = $data['status'];
        $id =$data['item_id'];
        $sql ="UPDATE item SET stock = stock +'$qty' WHERE item_id ='$id'";
        // var_dump($sql);
    ...
    function update_stock_laptop_dipinjam($data)
    {
        $qty = $data['status'];
        $id = $data['item_id'];
        $sql ="UPDATE item SET stock = stock -'$qty' WHERE item_id ='$id'";
